...
class ModflowSwfnam(mfpackage.MFPackage):
    """
    ModflowSwfnam defines a nam package within a swf6 model.
    ...
    list : string
        * list (string) is name of the listing file to create for this SWF
          model. If not specified, then the name of the list file will be the
          basename of the SWF model name file and the '.lst' extension. For
          example, if the SWF name file is called "my.model.nam" then the list
    ...
    newtonoptions : [under_relaxation]
        * under_relaxation (string) keyword that indicates whether the surface
          water stage in a reach will be under-relaxed when water levels fall
          below the bottom of the model below any given cell. By default,
    ...
    packages : [ftype, fname, pname]
        * ftype (string) is the file type, which must be one of the following
          character values shown in table ref{table:ftype-swf}. Ftype may be
        ...
        * pname (string) is the user-defined name for the package. PNAME is
          ...
          SWF Model. If PNAME is specified for a stress package, then PNAME
    ...
    """
    packages = ListTemplateGenerator(('swf6', 'nam', 'packages',
                                      'packages'))
    package_abbr = "swfnam"
    _package_type = "nam"
    dfn_file_name = "swf-nam.dfn"
