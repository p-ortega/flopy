...
class ModflowSwfflw(mfpackage.MFPackage):
    """
    ModflowSwfflw defines a flw package within a swf6 model.
    ...
    auxmultname : string
        * auxmultname (string) name of auxiliary variable to be used as
          multiplier of flow rate.
    boundnames : boolean
        * boundnames (boolean) keyword to indicate that boundary names may be
          provided with the list of inflow cells.
    print_input : boolean
        * print_input (boolean) keyword to indicate that the list of inflow
          information will be written to the listing file immediately after it
          is read.
    print_flows : boolean
        * print_flows (boolean) keyword to indicate that the list of inflow
          ...
          stress period.
    save_flows : boolean
        * save_flows (boolean) keyword to indicate that inflow flow terms will
    ...
    maxbound : integer
        * maxbound (integer) integer value specifying the maximum number of
          inflow cells that will be specified for use during any stress period.
    stress_period_data : [cellid, q, aux, boundname]
        ...
        * q (double) is the volumetric inflow rate. A positive value indicates
          inflow to the stream. Negative values are not allows. If the Options
        ...
        * aux (double) represents the values of the auxiliary variables for
          each inflow. The values of auxiliary variables must be present for
          each inflow. The values must be specified in the order of the
          ...
          numeric value.
        * boundname (string) name of the inflow cell. BOUNDNAME is an ASCII
    ...
    """
    auxiliary = ListTemplateGenerator(('swf6', 'flw', 'options',
                                       'auxiliary'))
    ts_filerecord = ListTemplateGenerator(('swf6', 'flw', 'options',
                                           'ts_filerecord'))
    obs_filerecord = ListTemplateGenerator(('swf6', 'flw', 'options',
                                            'obs_filerecord'))
    stress_period_data = ListTemplateGenerator(('swf6', 'flw', 'period',
                                                'stress_period_data'))
    package_abbr = "swfflw"
    _package_type = "flw"
    dfn_file_name = "swf-flw.dfn"
    ...
    def __init__(self, model, loading_package=False, auxiliary=None,
        ...
        super().__init__(model, "flw", filename, pname,
