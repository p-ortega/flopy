...
class ModflowGweuze(mfpackage.MFPackage):
    """
    ModflowGweuze defines a uze package within a gwe6 model.
    ...
    print_temperature : boolean
        * print_temperature (boolean) keyword to indicate that the list of UZF
          cell temperatures will be printed to the listing file for every
    ...
    packagedata : [uzfno, strt, aux, boundname]
        * uzfno (integer) integer value that defines the UZF cell number
          ...
          cell or the program will terminate with an error. The program also
          will terminate with an error if information for a UZF cell is
    ...
    uzeperioddata : [uzfno, uzesetting]
        * uzfno (integer) integer value that defines the UZF cell number
          ...
          index variables.
        * uzesetting (keystring) line of information that is parsed into a
          keyword and values. Keyword values that can be used to start the
          UZESETTING string include: STATUS, TEMPERATURE, INFILTRATION, UZET,
          and AUXILIARY. These settings are used to assign the temperature
          associated with the corresponding flow terms. Temperatures cannot be
          specified for all flow terms.
            status : [string]
                * status (string) keyword option to define UZF cell status.
                  STATUS can be ACTIVE, INACTIVE, or CONSTANT. By default,
                  STATUS is ACTIVE, which means that temperature will be
                  calculated for the UZF cell. If a UZF cell is inactive, then
                  there will be no energy fluxes into or out of the UZF cell
                  and the inactive value will be written for the UZF cell
                  temperature. If a UZF cell is constant, then the temperature
                  for the UZF cell will be fixed at the user specified value.
            temperature : [string]
                * temperature (string) real or character value that defines the
                  temperature for the unsaturated zone flow cell. The specified
                  TEMPERATURE is only applied if the unsaturated zone flow cell
                  is a constant temperature cell. If the Options block includes
            ...
            uzet : [string]
                * uzet (string) real or character value that states what
                  ...
                  is associated with evaporation. The evaporative losses from
                  the unsaturated zone moisture content will have an
                  evaporative cooling effect on the GWE cell from which the
                  evaporation originated. If this value is larger than 1, then
                  it will be reset to 1. If the Options block includes a
