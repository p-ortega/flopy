...
class ModflowRtmnam(mfpackage.MFPackage):
    """
    ModflowRtmnam defines a nam package within a rtm6 model.
    ...
    list : string
        * list (string) is name of the listing file to create for this RTM
          model. If not specified, then the name of the list file will be the
          basename of the RTM model name file and the '.lst' extension. For
          example, if the RTM name file is called "my.model.nam" then the list
    ...
    packages : [ftype, fname, pname]
        * ftype (string) is the file type, which must be one of the following
          character values shown in table ref{table:ftype-rtm}. Ftype may be
        ...
        * pname (string) is the user-defined name for the package. PNAME is
          ...
          RTM Model. If PNAME is specified for a stress package, then PNAME
    ...
    """
    packages = ListTemplateGenerator(('rtm6', 'nam', 'packages',
                                      'packages'))
    package_abbr = "rtmnam"
    _package_type = "nam"
    dfn_file_name = "rtm-nam.dfn"
